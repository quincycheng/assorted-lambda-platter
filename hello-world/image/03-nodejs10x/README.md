...
```bash
03-nodejs10x$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
03-nodejs10x$ sam local start-api
03-nodejs10x$ curl http://localhost:3000/
```
...
```bash
03-nodejs10x$ sam logs -n HelloWorldFunction --stack-name 03-nodejs10x --tail
```
...
```bash
aws cloudformation delete-stack --stack-name 03-nodejs10x
```
